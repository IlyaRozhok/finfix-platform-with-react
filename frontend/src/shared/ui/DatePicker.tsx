...
export const DatePicker: React.FC<DatePickerProps> = ({
  ...
  return (
    <div className={clsx("relative", className)} ref={containerRef}>
      ...
      >
        <span className={selectedDate ? "text-primary-background" : "text-primary-background/50"}>
      ...
      {/* Dropdown Calendar */}
      {isOpen && (
        <div className="absolute z-[100] bottom-full mb-2 w-full bg-white/95 backdrop-blur-xl rounded-2xl border border-white/50 shadow-2xl p-4">
          {/* Header */}
          ...
          <div className="grid grid-cols-7 gap-1 mb-2">
            {["Su", "Mo", "Tu", "We", "Th", "Fr", "Sa"].map(day => (
              <div key={day} className="text-center text-xs font-medium text-primary-background/70 py-2">
                {day}
              </div>
            ))}
          </div>
              ...
              return (
                <button
                  ...
                  className={clsx(
                    ...
                    {
                      "text-primary-background/30 border-transparent": !isCurrentMonth,
                      "text-primary-background hover:bg-white/20 border-white/20": isCurrentMonth && !isSelected,
                      "bg-blue-500/20 text-blue-100 border-blue-400/50 font-semibold": isSelected,
                      "ring-2 ring-blue-400/50 border-blue-400/50": isToday && !isSelected,
                    }
                  )}
                >
                  {day.getDate()}
